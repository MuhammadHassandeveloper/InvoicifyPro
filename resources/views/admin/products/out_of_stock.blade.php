...
@section('out-of-stock-product-list-page','active')
@section('style')
                    ...
                    <div class="card">
                        <div class="card-body">
                            <ul class="nav nav-tabs nav-tabs-custom mb-2 nav-success mb-3" role="tablist">
                                ...
                                <li class="nav-item">
                                    <a class="nav-link @yield('active-product-list-page')" href="{{ url('/admin/products/active') }}" role="tab" aria-selected="false">
                                        active
                                    </a>
                                </li>
                                <li class="nav-item">
                                    <a class="nav-link @yield('disabled-product-list-page')" href="{{ url('/admin/products/disabled') }}" role="tab" aria-selected="false">
                                        Disabled
                                    </a>
                                </li>

                                <li class="nav-item">
                                    <a class="nav-link @yield('low-stock-product-list-page')" href="{{ url('/admin/products/low-stock') }}" role="tab" aria-selected="false">
                                        Low  Stock
                                    </a>
                                </li>

                                <li class="nav-item">
                                    <a class="nav-link @yield('out-of-stock-product-list-page')" href="{{ url('/admin/products/out-of-stock') }}" role="tab" aria-selected="false">
                                        Out of Stock
                                    </a>
                                </li>
                                    ...
                                    @foreach ($products as $product)
                                        <tr id="product-{{ $product->id }}">
                                            <td>
                                                ...
                                                <a href="{{ route('admin.products.edit',$product->id) }}" class="text-body align-middle fw-medium">{{ $product->name }}</a>
                                            </td>
            ...
            <div class="modal-dialog modal-dialog-centered ">
                <div class="modal-content border-0">
                    ...
                    <div class="modal-body p-4">
                        <form method="post" action="{{ route('admin.products.updateStock') }}" autocomplete="off" id="brandform" class="needs-validation" enctype="multipart/form-data" novalidate="">
                            @csrf
                            @method('post')
    ...
    <script>
        ...
        function changeStatus(productId) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "/admin/products/status/" + productId,
                        type: 'post',
                        data: {
                            "_token": "{{ csrf_token() }}"
                        },
        ...
        function viewProduct(productId) {
            $.ajax({
                url: "/admin/products/show/" + productId,
                type: 'GET',
                success: function(response) {
